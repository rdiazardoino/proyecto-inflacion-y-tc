...
3. Archivos sueltos en el dominio clásico con naming predecible — así se resolvió
   el TC (SOAP) y así se intenta ahora la Encuesta de Expectativas: los PDFs viven
   en `.../Encuesta de Expectativas Econmicas/iees06i{MM}{YY}.pdf` (confirmado con
   dos ejemplos reales: `iees06i0426.pdf` = abril 2026, `iees06i1225.pdf` = dic
   2025). `descubrir_fuentes.py` ya intenta este patrón para los últimos 4 meses
   en cada corrida — si el próximo run trae los PDF, se calibra el parser (son PDF
   de texto, extraíbles con pdfplumber, no XLS).
